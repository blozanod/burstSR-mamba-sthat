...
# Vendored from goutamgmb/deep-burst-sr (dataset/synthetic_burst_val_set.py).
# Local adaptation: the `admin.environment` import was removed; `root` is now a
# required argument instead of falling back to env_settings(). See README.md in
# this folder for provenance.
...
class SyntheticBurstVal(torch.utils.data.Dataset):
    ...
    def __init__(self, root, initialize=True):
        ...
        """
        assert root is not None, 'SyntheticBurstVal requires an explicit dataset root'
        self.root = root
