...
| `offset_analysis.py` / `burst_ablation.py` | Distributed ablation tools; log to `analysis/outputs/ablation_logs/` by default. `offset_analysis.py` reads BurstAlign's per-level flow from the model's aux return and reports mean \|displacement\| in both level px and GT px; the levels compose, so read lv1 as the total. Pre-L6 numbers from this script are void (wrong DCN channel layout — see its docstring). `burst_ablation.py` has two modes: `two_pass` (normal vs. all-ref, the burst-utilization score) and `frame_drop` (PLAN.md L5 curves over N = 1, 2, 5, 9, 14 distinct frames, remaining slots ref-filled), on either `--dataset realbsr` or `--dataset synburst` (official val set). |
| `dcn_scatter_check.py` | Two correctness checks for the L6 flow plumbing: `--scatter` verifies that `scatter_flow` writes into the channels DCNv4's CUDA kernel reads as (dx, dy), by matching the DCN against `grid_sample`; `--sign` verifies that flow supervision targets `+flow_vectors` (needs `--dataroot`). Run both before a launch that changes alignment. |
